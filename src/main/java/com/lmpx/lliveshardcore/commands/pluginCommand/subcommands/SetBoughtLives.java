...
public class SetBoughtLives extends SubCommand implements LCommand {
  ...
  public String[] subPermissions() {
    return new String[]{
      "add", // Добавить чёта там
      "remove" // Отобрать чёта там уже хз если честно что
    };
  }
  ...
  public void onCommand(CommandSender sender, @NotNull String[] args) {
    ...
    try {
      if (!isAdd && !isRemove) {
        Main.sqLite.saveData(player, SQLite.KEY_BL, value);
      } else {
        if (isAdd) {
          Main.sqLite.saveData(player, SQLite.KEY_BL, Main.sqLite.getDataInt(player, SQLite.KEY_BL) + value);
        }
        if (isRemove) {
          Main.sqLite.saveData(player, SQLite.KEY_BL, Main.sqLite.getDataInt(player, SQLite.KEY_BL) - value);
        }
